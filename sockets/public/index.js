...
const closeChat = (selectorID) => {
  document
    .querySelector(`.tabs ul li[data-content='${selectorID}']`)
    .removeEventListener("click", selectedChat);
...
const chatTo = (data) => {
  ...
  if (existChat === EMPTY) {
    if (
      user.dataset.idhtml !== selectorID ||
      data.idDestinoHTML !== data.idOrigenHTML
    ) {
      ...
      document
        .querySelector(`.tabs ul li[data-content='${selectorID}']`)
        .addEventListener("click", selectedChat);
    } else {
...
const selectedChat = (evt) => {
  ...
  if (
    !Array.from(
      evt.currentTarget.children[0].querySelector("i").classList
    ).includes("hide")
  ) {
...
const sendMessage = (evt, status) => {
  if (evt.key === "Enter") {
    if (
      user.value.length >= LENGTH_MIN_USERNAME &&
      message.value.trim().length > EMPTY
    ) {
      const parent =
        document.querySelectorAll(".tabs a.active")[0].parentElement;
      ...
      if (content !== CHAT_GENERAL) {
        const color = document.querySelector(
          `#userConnected .chatUser[data-idhtml='${user.dataset.idhtml}']`
        ).style.color;
        const messageSend = document.querySelector(
          `.containerChats #${content}`
        );
      ...
      socket.emit(
        `message-chat-${isChatGeneral ? "general" : "private"}`,
        infoMensaje
      );
      message.value = "";
      currentlyChat.scrollTo(0, currentlyChat.scrollHeight);
    }
    user.classList[user.value.length >= LENGTH_MIN_USERNAME ? "remove" : "add"](
...
const reciveMessage = (data) => {
  const { color, idDestinoHTML, idOrigenHTML, userOrigen, message } = data;
  let activeChatID =
    document.querySelector(`#${idDestinoHTML}`) ||
    document.querySelector(`#${idOrigenHTML}`);
  if (!activeChatID) {
    chatTo(data);
    activeChatID =
      document.querySelector(`#${idDestinoHTML}`) ||
  ...
  const newMessage =
    document.querySelector(".tabs a.active").parentElement.dataset.content;
  if (newMessage !== activeChatID.id && newMessage !== idDestinoHTML) {
    const destino = document.querySelector(
      `.tabs li[data-content='${idDestinoHTML}'] i`
    );
    const origen = document.querySelector(
      `.tabs li[data-content='${idOrigenHTML}'] i`
    );
...
const connectedToServer = (evt) => {
  ...
  if (target.value.length >= LENGTH_MIN_USERNAME) {
    ...
    socket.on("error-registered-user", (payload) =>
      errorRegisteredUser(payload, evt)
    );
...
const uploadProgress = (data) => {
  ...
  infoProgress.innerHTML = `${currentlySize.toFixed(2)} MB ${totalSize.toFixed(
    2
  )} MB ${porcent} %`;
};
 ...
 */
const generateIDHtml = () =>
  new Date()
    .getTime()
    .toString()
    .split("")
    .map((i) => String.fromCharCode(parseInt(i) + 65))
    .join("");
