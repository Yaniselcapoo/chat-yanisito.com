...
const MIME_TYPE = ["image/gif", "image/jpeg", "image/png"];
const LITERAL = {
  fileNotAllowed: `El fichero que intenta subir no esta permitido. Los tipos permitidos son los siguientes: <b>[${MIME_TYPE.join(
    ", "
  )}]</b>`,
...
app.post("/upload-file", (req, res) => {
  ...
  form.on("end", () => {
    res.status(error ? 400 : 200).send({
      ...
      statusMessage: error
        ? LITERAL.fileNotAllowed
        : `${req.protocol}://${req.hostname}:${PORT}/upload-file/${fileName}`,
      path: null,
    });
  });
  form.on("progress", (bytesReceived, bytesExpected) => {
    io.to(who).emit("upload-progress", {
      recived: bytesReceived,
      total: bytesExpected,
      who,
    });
  ...
  form.on("error", () => {
    res
      .status(404)
      .send({ statusCode: 404, statusMessage: LITERAL.maxSize, path: null });
  });
});
...
const connection = async (payload, socket, color) => {
  ...
  if (error) {
    io.to(idUser).emit("error-registered-user", {
...
const writeClient = (payload) => {
  ...
  io.emit(
    "client-been-writing",
    data ? `El usuario ${data} esta escribiendo...` : ""
  );
};
...
io.on("connection", (socket) => {
  ...
  socket.on("connected-to-server", (payload) =>
    connection(payload, socket, color)
  );
  socket.on("disconnect", () => closeConnection(socket));
  socket.on("write-client", writeClient);
  socket.on("message-chat-general", (payload) =>
    messageChat(payload, color, "general")
  );
  socket.on("message-chat-private", (payload) =>
    messageChat(payload, color, "private")
  );
